...
void* testCS(void *threadId)
{
	...
	for(long long i=0;i<k;i++)
	{
		...
		while(!lock.compare_exchange_weak(expectedLockValue, 1))
		{
			expectedLockValue = 0;
		}
		...
		waitingTime += (double)currentTime2 - (double)currentTime1; 
...
int main()
{
	...
	output1 = fopen("CAS.txt", "w");
	output2 = fopen("performanceCAS.txt", "w");
	input>>n>>k>>l1>>l2;
	pthread_t threads[n];
	pthread_attr_t attr;
	pthread_attr_init(&attr); //setting attributes of threads
	fprintf(output1, "CAS ME Output:\n");
	//creating threads
