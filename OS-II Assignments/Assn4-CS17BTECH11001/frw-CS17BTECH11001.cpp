...
sem_t rw_mutex,mutex,order_mutex; //semaphores for reader writer lock and reader lock
...
int main()
{
	...
	output1 = fopen("FairRW-log.txt", "w");
	output2 = fopen("Fair-RW.txt", "w");
	...
	sem_init(&order_mutex,0,1); //initializing order mutex lock to 1
